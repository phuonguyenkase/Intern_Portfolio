...
try:
    ...
    filtered_stocks = [
        ...
        if s.get('industry_lv1') == 'Industrials' and s.get('industry_lv2') == 'Industrial Goods & Services'
    ]
    ...
    goods_and_services_symbols = [s['symbol'] for s in filtered_stocks]
...
class RelativeValuationScorer:
    ...
    def evaluate_criterion(self, goods_and_services_df, criterion, all_goods_and_services_data, peer_data_cache=None):
        try:
            crit_type = criterion['type']
            ...
            if crit_type == 'ratio_threshold':
                ...
                # Use pre-computed peer_data_cache if available
                cache_key = f"{criterion['pb_code']}/{criterion['roe_code']}"
    ...
    def score_goods_and_services(self, symbol, goods_and_services_df, all_goods_and_services_data, peer_data_cache=None):
        results = {}
        points = 0.0
        ...
        for crit in self.criteria:
            if crit.get('must_have', False):
                result = self.evaluate_criterion(goods_and_services_df, crit, all_goods_and_services_data, peer_data_cache)
                must_have_results.append(result['pass'])
                results[f"C{crit['id']}"] = result
...
try:
    ...
    val_peer_data_cache = {}
    val_metric_codes = ['ryd25', 'ryd21']  # P/B and P/E
    for code in val_metric_codes:
        val_peer_data_cache[code] = [val_scorer.get_metric_value(df, code) for df in goods_and_services_ratios_data.values()]
    
    # Pre-compute peer ratios for ratio_threshold
    pb_values = [val_scorer.get_metric_value(df, 'ryd25') for df in goods_and_services_ratios_data.values()]
    roe_values = [val_scorer.get_metric_value(df, 'ryq12') for df in goods_and_services_ratios_data.values()]
    peer_ratios = []
    for pb, roe in zip(pb_values, roe_values):
        if pb is not None and roe is not None and roe != 0 and roe >= 0.08:
            peer_ratios.append(pb / roe)
    val_peer_data_cache['ryd25/ryq12'] = peer_ratios
    
    val_scores = {}

    for symbol, goods_and_services_df in goods_and_services_ratios_data.items():
        val_scores[symbol] = val_scorer.score_goods_and_services(symbol, goods_and_services_df, goods_and_services_ratios_data, val_peer_data_cache)    
